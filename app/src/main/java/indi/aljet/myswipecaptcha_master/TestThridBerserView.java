...
public class TestThridBerserView extends View {
    ...
    public TestThridBerserView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        drawPartCircle(new PointF(mCaptchaX + mCaptchaWidth,
                ...
                mCaptchaPath,false);
        mCaptchaPath.lineTo(mCaptchaX + mCaptchaWidth,
                mCaptchaY + mCaptchaHeight);//节点
        mCaptchaPath.lineTo(mCaptchaX + mCaptchaWidth - gap,
                mCaptchaY + mCaptchaHeight);

        drawPartCircle(new PointF(mCaptchaX + mCaptchaWidth
        - gap,mCaptchaY + mCaptchaHeight),
                new PointF(mCaptchaX + mCaptchaWidth -
                gap - gap,mCaptchaY + mCaptchaHeight),
                mCaptchaPath,false);
        ...
        drawPartCircle(new PointF(mCaptchaX,
                mCaptchaY + mCaptchaHeight - gap),
                new PointF(mCaptchaX,mCaptchaY + mCaptchaHeight
                 - gap * 2),mCaptchaPath,false);
     ...
     * @param outer 是否凸半圆
     */
    private void drawPartCircle(PointF start ,PointF end,
                                Path path,boolean outer){
        float c = 0.551915024494f;
        //中点
        PointF middle = new PointF(start.x + (end.x -
        start.x) / 2,start.y + (end.y - start.y )
        / 2);
        //半径
        float r1 = (float) Math.sqrt(Math.pow((middle.x -
        start.x),2) + Math.pow((middle.y - start.y),2));
        //gap值
        float gap1 = r1 * c;
        ...
        if(start.x == end.x){
            ...
            if(outer){
                //凸的 两个半圆
                path.cubicTo(start.x + gap1 * flag,
                        start.y,middle.x + r1 * flag,
                        middle.y - gap1 * flag,
                        middle.x + r1 * flag,
                        middle.y);
