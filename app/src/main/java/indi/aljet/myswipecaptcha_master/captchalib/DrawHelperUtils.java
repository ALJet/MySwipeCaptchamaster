package indi.aljet.myswipecaptcha_master.captchalib;
...
public class DrawHelperUtils {
     ...
     * @param outer 是否凸半圆
     */
    public static void drawPartCircle(PointF start,
                                      PointF end,
                                      Path path,
                                      boolean outer){
        float c = 0.551915024494f;
        //中点
        PointF middle = new PointF(start.x + (end.x - start
        .x) / 2,start.y + (end.y - start.y) / 2);
        //半径   ????
        ...
        if(start.x == end.x){
            ...
            if(outer){
                ...
                path.cubicTo(start.x,
                        ...
                        middle.x,
                        middle.y - r1 * flag);
                path.cubicTo(middle.x + gap1 * flag,
                        middle.y - r1 * flag,
                        end.x,
                        end.y - gap1 * flag,
                        end.x,end.y);
